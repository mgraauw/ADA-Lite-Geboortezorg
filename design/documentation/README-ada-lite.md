...
Mocht het toch noodzakelijk zijn dan kunnen de volledige specificaties gevonden worden in `design/specs-full`.
...
```
java -cp "pad-naar-saxon-installatie-directory" net.sf.saxon.Transform "-s:pad-naar-invoer-bestand" "-xsl:pad-naar-adalite2ada-full.xsl" "-o:pad-naar-uitvoer-bestand" "dref-rtd=file:/pad-naar-specificatie"
```
...
- Voor het uitvoeren van "losse" transformaties en validaties wordt [Saxon](http://saxonica.com) aangeraden. De HE (Home Edition, gratis) version is voldoende.

We gaan er in de verdere beschrijvingen van uit dat Saxon is geïnstalleerd volgens de instructies zoals hieronder. 
...
Of maak een omgevings variabele `CLASSPATH` waarin je deze directory opneemt.
...
```
java -cp "pad-naar-saxon-installatie-directory" com.saxonica.Validate "-s:pad-naar-te-valideren-bestand" "-xsd:pad-naar-schema-bestand"
```
...
```
java -cp "pad-naar-saxon-installatie-directory" net.sf.saxon.Transform "-s:pad-naar-invoer-bestand" "-xsl:pad-naar-stylesheet" "-o:pad-naar-uitvoer-bestand"
```

### <a name="schematron-validatie"></a>Schematron validatie

Validatie met Schematron is op de directe manier mogelijk zoals dat met schemas wel kan. Opties zijn:
